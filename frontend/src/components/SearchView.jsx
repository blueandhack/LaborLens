...
const SearchView = () => {
    ...
    const initialDeterminationYear = searchParams.get('determinationYear') || '';
    ...
    const [determinationYear, setDeterminationYear] = useState(initialDeterminationYear);
    ...
    const fetchYears = async () => {
        try {
            const { data } = await axios.get(`/api/search/years`);
    ...
    const updateURL = (newCompany, newLocation, newCaseNumber, newJobTitle, newDeterminationYear, newReceivedYear, newPage, newLimit) => {
        ...
        if (newDeterminationYear) params.set('determinationYear', newDeterminationYear);
    ...
    useEffect(() => {
        // Fetch the available dropdown years
        fetchYears();

        // If we load the page directly with params, fetch immediately
        if (company || location || caseNumber || jobTitle || determinationYear || receivedYear || currentPage > 1 || limit !== 12) {
            fetchResults(currentPage, limit);
        } else {
            fetchResults(1, limit); // empty initial fetch
        }
    ...
    const handleClearClick = () => {
        ...
        setCaseNumber('');
        setJobTitle('');
        setDeterminationYear('');
        setReceivedYear('');
        setLimit(12);
        updateURL('', '', '', '', '', '', 1, 12);

        setIsLoading(true);
        axios.get(`/api/search`, { params: { page: 1, limit: 12 } })
            .then(({ data }) => {
                setResults(data.cases);
                setTotalPages(data.totalPages);
                setCurrentPage(data.currentPage);
    ...
    const handlePageChange = (newPage) => {
        if (newPage >= 1 && newPage <= totalPages) {
            updateURL(company, location, caseNumber, jobTitle, determinationYear, receivedYear, newPage, limit);
        ...
        <div>
            <div className="search-bar-container glass-panel" style={{ padding: '20px', flexDirection: 'column', gap: '15px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', width: '100%' }}>
                    ...
                    <select
                        className="search-input"
                        value={determinationYear}
                        onChange={(e) => setDeterminationYear(e.target.value)}
                        style={{ cursor: 'pointer', appearance: 'none' }}
                    >
                        <option value="">Determination Year</option>
                        {availableYears.map(year => (
                            <option key={`det-${year}`} value={year}>{year}</option>
                        ))}
                    </select>
                    <select
                        className="search-input"
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            setLimit(newLimit);
                            updateURL(company, location, caseNumber, jobTitle, determinationYear, receivedYear, 1, newLimit);
                            ...
                            <div className="case-card glass-panel" style={{ height: '100%' }}>
                                <div className="card-header">
                                    <span className="company-name">
                                        {caseItem.EMPLOYER_LEGAL_BUSINESS_NAME || 'Unknown Company'}
                                    </span>
                                ...
                                <div className="card-body">
                                    ...
                                    <div className="info-row">
                                        ...
                                        <span style={{ textAlign: 'right' }}>{caseItem.EMPLOYER_CITY || 'N/A'}, {caseItem.EMPLOYER_STATE || 'N/A'}</span>
                                    </div>
                                    ...
                                    <div className="info-row">
                                        ...
                                        <span style={{ textAlign: 'right' }}>{caseItem.NAICS_CODE || 'N/A'}</span>
                                    </div>
                                    ...
                                    <div className="info-row">
                                        <span className="info-label">Determination Date</span>
                                        <span style={{ textAlign: 'right' }}>
                                            {caseItem.DETERMINATION_DATE ? new Date(caseItem.DETERMINATION_DATE).toLocaleDateString() : 'N/A'}
                                        </span>
                                    </div>
                                    <div className="info-row">
                                        <span className="info-label">Expiration Date</span>
                                        <span style={{ textAlign: 'right' }}>
                                            {caseItem.PWD_WAGE_EXPIRATION_DATE ? new Date(caseItem.PWD_WAGE_EXPIRATION_DATE).toLocaleDateString() : 'N/A'}
                                        </span>
                                    </div>
                                    <div className="info-row">
