...
const SearchView = () => {
    ...
    const fetchResults = async (page = 1, currentLimit = limit) => {
        ...
        try {
            const { data } = await axios.get('/api/search', {
                params: { company, location, caseNumber, jobTitle, determinationYear, receivedYear, page, limit: currentLimit }
            });
    ...
    const updateURL = (c, l, cn, jt, dy, ry, page, lim) => {
        ...
        if (dy)   params.set('determinationYear', dy);
    ...
    const handleClearClick = () => {
        ...
        setDeterminationYear(''); setReceivedYear(''); setLimit(12);
        updateURL('', '', '', '', '', '', 1, 12);
        setIsLoading(true);
        ...
        <div>
            ...
            <div className="search-bar-container glass-panel" style={{ padding: '18px 20px', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '10px', width: '100%' }}>
                    ...
                    <select className="search-input" value={determinationYear} onChange={e => setDeterminationYear(e.target.value)}>
                        <option value="">Determination Year</option>
                        {availableYears.map(y => <option key={`det-${y}`} value={y}>{y}</option>)}
                    </select>
            ...
            <div className="results-info-bar">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <select className="search-input" value={limit} disabled={isLoading}
                        onChange={e => { const v = parseInt(e.target.value); setLimit(v); updateURL(company, location, caseNumber, jobTitle, determinationYear, receivedYear, 1, v); fetchResults(1, v); }}
            ...
            {isLoading ? (
                ...
                <div className="results-grid">
                    {results.map(c => {
                        ...
                        const location    = [c.EMPLOYER_CITY, c.EMPLOYER_STATE].filter(Boolean).join(', ') || 'N/A';
                        return (
                                ...
                                <div className={`case-card glass-panel status-${statusClass}`}>
                                    <div className="card-header">
                                        <span className="company-name">
                                            {c.EMPLOYER_LEGAL_BUSINESS_NAME || 'Unknown Company'}
                                        </span>
                                    ...
                                    <div className="card-body">
                                        ...
                                        <div className="info-row">
                                            <span className="info-label">SOC Code</span>
                                            <span className="info-value mono" title={c.PWD_SOC_TITLE}>{c.PWD_SOC_CODE || 'N/A'}</span>
                                        </div>
                                        <div className="info-row">
                                            <span className="info-label">Det. Date</span>
                                            <span className="info-value">
                                                {c.DETERMINATION_DATE ? new Date(c.DETERMINATION_DATE).toLocaleDateString() : 'N/A'}
                                            </span>
                                        </div>
                                    </div>
